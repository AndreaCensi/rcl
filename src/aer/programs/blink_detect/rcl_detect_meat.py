...
class MultipleDetector(object):
    def __init__(self, log, pipeline, sigma, outdir, tracks_filename,
        ...
        if False:
            ...
            self.transitions = aer_pipeline_transitions1(raw_sequence,
                                                         pipeline)
        else:
    ...
    def two_phase_estimation(self, timestamp, triggered):
        ...
        distances = [d(p1, p2) for p1, p2 in
                     itertools.combinations(occupied, 2)]
        if distances:
...
def remove_occupied(accum, occupied, distance):
    """ Zeros the elements of the accum in the radius around the given
    points """
    accum = accum.copy()
    for x, y in occupied:
        x = int(x)
